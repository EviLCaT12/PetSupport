...
public class EditMessageHandler : ICommandHandler<EditMessageCommand>
{
    ...
    public async Task<UnitResult<ErrorList>> HandleAsync(EditMessageCommand command, CancellationToken cancellationToken)
    {
        ...
        var newText = Text.Create(command.NewText).Value;
        discussion.EditComment(message, newText);
